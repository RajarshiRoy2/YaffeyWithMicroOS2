...
/*
*********************************************************************************************************
*                                   SET MUTEX NESTING ATTRIBUTE
* Description: This  function sets or clears the mutex nesting attribute. Mutexes are created as not
*              nested, and the nested attribute must be set by calling this function when the mutex
*              is not currently owned. If the mutex is owned and a pend is requested, the pend will 
*              return immediately and the nesting count will be incremented. The next post will not
*              release the mutex, but will decrement the nesting count. A nested mutex can be acquired 
*              by the current owner up to 255 times without error. It must be posted an equal number 
*              of times before the mutex is released. 
*
* Arguments  : pevent     is a pointer to the event control block
*
*              err        indicates whether the nesting attribute should be set (non-zero) or cleared (0).
* Returns    : OS_NO_ERR  if the mutex attribute was set or cleared
*              OS_RESOURCE_NOT_AVAILABLE if the mutex was owned at the time the call was made.
*/
INT8U OSMutexSetNesting(OS_EVENT *pevent, INT8U nesting)
{
    ...
    if (pevent->OSEventType != OS_EVENT_TYPE_MUTEX && pevent->OSEventType != OS_EVENT_TYPE_NESTED_MUTEX) 
    	return OS_ERR_EVENT_TYPE;
    
    OS_ENTER_CRITICAL();
    if ( (pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) != OS_MUTEX_AVAILABLE) 
	    ret = OS_RESOURCE_NOT_AVAILABLE;
    else
    {    	
    	pevent->OSEventType = (INT8U)(nesting ? OS_EVENT_TYPE_NESTED_MUTEX : OS_EVENT_TYPE_MUTEX);
    	ret = OS_NO_ERR;
    }
    OS_EXIT_CRITICAL();
...
INT8U  OSMutexAccept (OS_EVENT *pevent, INT8U *err)
{
    ...
    if (pevent->OSEventType != OS_EVENT_TYPE_MUTEX &&
        pevent->OSEventType != OS_EVENT_TYPE_NESTED_MUTEX) {  /* Validate event block type                    */
    ...
    if ((pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        pevent->OSEventNesting = 1;                    /* TN -- Initialize nesting count               */
        pevent->OSEventPtr  = (void *)OSTCBCur;        /*      Link TCB of task owning Mutex           */
        OS_EXIT_CRITICAL();
        *err = OS_NO_ERR;
        return (1);
    }
    if(pevent->OSEventType == OS_EVENT_TYPE_NESTED_MUTEX && pevent->OSEventPtr == OSTCBCur)		
    {												  /* TN -- Is mutex owned by current task? */
    	if(pevent->OSEventNesting != 0xff)
    	{
    		++pevent->OSEventNesting;		/* TN -- Increment nesting count and return success */
    		OS_EXIT_CRITICAL();
    		*err = OS_NO_ERR;
    		return (1);
    	}
...
void  OSMutexPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        if (OSTCBCur->OSTCBPrio <= pip) {                  /*      PIP 'must' have a SMALLER prio ...  */
        	pevent->OSEventCnt |= OS_MUTEX_AVAILABLE;      /*      ... than current task!              */
            OS_EXIT_CRITICAL();							   /* TN -- mark mutex available               */
    ...
    {			                                /* TN -- Is mutex owned by current task? */
    	...
    	OS_EXIT_CRITICAL();						/* TN -- Nesting overflow, can't acquire. */ 
    	*err = OS_NESTING_ERR;
    	return;
    }
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if(pevent->OSEventType == OS_EVENT_TYPE_NESTED_MUTEX && pevent->OSEventNesting != 0)
    {												  /* TN -- Is mutex nested? */
     	if(--pevent->OSEventNesting != 0)			  /* TN -- Decrement nesting count */
    	{											  /* TN -- If still nested, return without release */
    		OS_EXIT_CRITICAL();
    		return (OS_NO_ERR);
    	}
    }
...
#if OS_LOWEST_PRIO <= 63
        OSTCBCur->OSTCBY            = (INT8U)(prio >> 3);
        OSTCBCur->OSTCBX            = (INT8U)(prio & 0x07);
        OSTCBCur->OSTCBBitY         = (INT8U)(1 << OSTCBCur->OSTCBY);
        OSTCBCur->OSTCBBitX         = (INT8U)(1 << OSTCBCur->OSTCBX);
#else
        OSTCBCur->OSTCBY            = (INT8U)((prio >> 4) & 0xFF);
        OSTCBCur->OSTCBX            = (INT8U)(prio & 0x0F);
